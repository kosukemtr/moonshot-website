...
- このディレクトリでPythonスクリプトや一時的なPython解析を実行する場合は、原則として `uv run python ...` を使ってください。
...
- arXiv論文の図から数値を読む必要がある場合は、PDF本文だけでなくarXiv source、同梱LaTeX、図PDF、公開データリポジトリ、著者公開コードを探し、利用できる元データやベクター図がないか確認してください。生データがなく図から座標抽出した場合は、その旨と有効数字の限界を `備考` に明記してください。
